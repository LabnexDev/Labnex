...
interface ModalProps {
  ...
  children: ReactNode;
  footer?: ReactNode;
  size?: 'sm' | 'md' | 'lg' | 'xl' | '2xl'; // Modal width
}
...
export const Modal: React.FC<ModalProps> = ({
  ...
  const modalContent = (
    <div 
      className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}
    >
      <div 
        className={`relative w-full ${sizeClasses[size]} rounded-xl shadow-2xl bg-[var(--lnx-surface)] dark:bg-gradient-to-br dark:from-gray-800 dark:to-gray-900 border border-[var(--lnx-border)] dark:border-gray-700/50 flex flex-col max-h-[90vh] transition-all duration-300 ease-in-out transform scale-95 opacity-0 animate-modal-appear`}
